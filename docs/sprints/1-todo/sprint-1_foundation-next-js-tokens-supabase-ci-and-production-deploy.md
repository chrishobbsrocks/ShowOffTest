...
9. **Migration path proven on production (NFR-5, D-50).** Schema changes live as versioned SQL migrations committed under `supabase/migrations/`, laid out as the Supabase CLI expects (`supabase/config.toml` present). This sprint adds one baseline migration that creates a SQL function `public.health_check()` returning `true`, executable by the anon role and nothing else. **Migrations reach production through Supabase's GitHub integration, applying on push to `main`** — the operator's decision. The operator connects the integration in the Supabase dashboard; Dev Team supplies the repository layout it needs and a short step-by-step for the operator. *Flagged assumptions, Dev Team must verify against Supabase's current documentation before building on them, and report back rather than work around them if either is false:* (a) production deploys of migrations from `main` work with Supabase Branching (per-branch preview databases) turned **off**, because the PRD allows one production database and no per-branch database; (b) the feature is available on the project's current plan. The procedure, including what to check when a migration fails to apply, is written into `CLAUDE.md`'s project standards so every later sprint follows it. No role other than the operator holds a production database secret. The baseline migration reaches production through the integration before the live test.
...
9. QA1: `supabase/migrations/` holds exactly one migration; it creates `health_check()`, revokes execute from `public`, and grants execute to `anon` only. The GitHub-integration procedure is in `CLAUDE.md`, names the check for a failed apply, and needs no secret held by anyone but the operator. Dev Team's notes record how assumptions (a) and (b) were verified. LiveQA: the Supabase integration shows the baseline migration applied from the deployed commit on `main`, and Branching is off (operator shares the dashboard view if LiveQA cannot see it).
...
- External: operator setup as of 2026-09-12. **Verified:** Supabase and Vercel projects exist; Docker Desktop is installed and its engine reachable; the four D-56 variable names are set in Vercel, Production only (values not seen, by design). The Supabase project `ShowOffTest` is connected to the repository `chrishobbsrocks/ShowOffTest` (operator's screenshot of the organisation's Integrations page). **Not yet verified:** that connection's settings — working directory `.`, Deploy to production on for `main`, automatic branching off. The Supabase organisation is on the **Free** plan, which makes flagged assumption (b) in requirement 9 the most likely point of failure. Before Pipeman's first push of this sprint, Dev Team confirms those settings with the operator (or LiveQA views them read-only, D-54); if they are not in place, that is an operator setup item, not a code defect.
...
- The GitHub integration may need Branching or a paid plan — requirement 9 flags both as assumptions to verify first and report, not work around.
